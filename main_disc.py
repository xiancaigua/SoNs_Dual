...
# ======== 世界类 ========
class DiscreteWorld:
    def __init__(self, map_mode="random", map_id=2, seed=42):
        if seed is not None:
            random.seed(seed)
    ...
    def generate_fixed_map(self, map_id=1):
        ...
        if map_id == 1:
            ...
            self.true_grid[30:45, 10:25] = OBSTACLE
            self.true_grid[10:15, 30:40] = OBSTACLE
            self.true_grid[30:45, 30:40] = OBSTACLE
            self.true_grid[-10:-1, -10:-1] = OBSTACLE
        ...
        # ---------- 2️⃣ 迷宫场景 ----------
            ...
            # 横竖交错迷宫通道
            for y in range(5, GRID_H-5, 4):
                self.true_grid[y:y+1, 3:GRID_W-3] = OBSTACLE
            for x in range(5, GRID_W-5, 6):
                self.true_grid[3:GRID_H-3, x:x+1] = OBSTACLE
            # 开几个通道口
            for x in range(6, GRID_W-6, 12):
                self.true_grid[10, x] = FREE
                self.true_grid[20, x] = FREE
            for y in range(8, GRID_H-8, 8):
                self.true_grid[y, 12] = FREE
                self.true_grid[y, 24] = FREE
            # 添加危险区域
            for _ in range(20):
                x, y = random.randint(5, GRID_W-5), random.randint(5, GRID_H-5)
                if self.true_grid[y, x] == FREE:
                    self.true_grid[y, x] = DANGER
            victim_pos = (GRID_W-6, GRID_H-6)
            small_starts = [(2, 2), (3, 2), (2, 3), (3, 3)]
            large_starts = [(1, 1)]
        ...
        # ---------- 3️⃣ 危险长管道 ----------
            ...
            self.true_grid[:, GRID_W-1] = OBSTACLE
            self.true_grid[0, :] = OBSTACLE
            self.true_grid[GRID_H-1, :] = OBSTACLE
            self.true_grid[10:GRID_H-10, 15] = OBSTACLE
            self.true_grid[10:GRID_H-10, 25] = OBSTACLE
            # 中间走廊充满危险区
            for y in range(12, GRID_H-12):
                if random.random() < 0.4:
                    self.true_grid[y, 20] = DANGER
            victim_pos = (20, GRID_H-3)
            small_starts = [(20, 3), (21, 3), (19, 3)]
            large_starts = [(18, 2)]
        ...
        # ---------- 4️⃣ 森林障碍随机分布 ----------
            ...
            for _ in range(100):
                ...
                self.true_grid[y, x] = OBSTACLE
            for _ in range(40):
                x, y = random.randint(1, GRID_W-2), random.randint(1, GRID_H-2)
                self.true_grid[y, x] = DANGER
            victim_pos = (GRID_W-5, GRID_H-5)
            small_starts = [(3, 3), (4, 3), (3, 4), (5, 4)]
            large_starts = [(2, 2)]
        ...
        # ---------- 5️⃣ 多房间结构 ----------
            ...
            self.true_grid[:, GRID_W-1] = OBSTACLE
            # 划分房间
            for x in range(8, GRID_W-8, 8):
                self.true_grid[:, x] = OBSTACLE
            for y in range(6, GRID_H-6, 6):
                self.true_grid[y, :] = OBSTACLE
            # 打通部分房门
            for _ in range(12):
                x, y = random.randint(2, GRID_W-3), random.randint(2, GRID_H-3)
                self.true_grid[y, x] = FREE
            # 危险区域散布
            for _ in range(25):
                x, y = random.randint(2, GRID_W-3), random.randint(2, GRID_H-3)
                if self.true_grid[y, x] == FREE:
                    self.true_grid[y, x] = DANGER
            victim_pos = (GRID_W-6, GRID_H-6)
            small_starts = [(2, 2), (4, 2), (2, 4), (3, 3)]
            large_starts = [(1, 1)]

        else:
            raise ValueError(f"Unsupported map_id {map_id}")
