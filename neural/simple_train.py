...
class SimpleBrainFitter(nn.Module):
    def __init__(self, state_dim=12, map_h=35, map_w=50, num_goals=3):
        super().__init__()
    ...
    def forward(self, state, m):
        ...
        return self.head(combined).view(-1, self.num_goals, 2)
...
class BrainNPZDataset(Dataset):
    def __init__(self, npz_path):
        """
        加载汇总后的 npz 数据文件
        :param npz_path: training_data.npz 的路径
        """
        # 加载数据
        data = np.load(npz_path)
        
        # 提取状态 (N, 6) 和 地图 (N, 1, 50, 50)
        self.states = torch.from_numpy(data['states']).float()
        self.maps = torch.from_numpy(data['maps']).float()
        
        # 处理目标标签 (N, 2)
        goals = data['goals']
        
        # 执行归一化：与 neural_brain_logic.py 的反归一化逻辑对应
        # x 坐标除以 1000.0, y 坐标除以 700.0
        goals[:, 0] /= 1000.0
        goals[:, 1] /= 700.0
        
        self.goals = torch.from_numpy(goals).float()
        
        print(f"✅ 成功加载数据集: {len(self.states)} 条样本")
    ...
    def __getitem__(self, idx):
        """
        返回单条数据供 DataLoader 使用
        """
        return self.states[idx], self.maps[idx], self.goals[idx]

# ==========================================
# 🚀 3. 训练脚本
# ==========================================
def train(data_path="./neural/brain_dataset/training_data.npz", epochs=200000):
    writer = SummaryWriter('./neural/runs/brain_fitter_experiment_2') # 创建记录器
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"使用设备: {device}")

    # 加载数据
    dataset = BrainNPZDataset(data_path)
    loader = DataLoader(dataset, batch_size=32, shuffle=True)
    
    model = SimpleBrainFitter(state_dim=6).to(device)
    ...
    # 保存模型
    if not os.path.exists("./neural/models"):
        os.makedirs("./neural/models")
    torch.save(model.state_dict(), "./neural/models/brain_model.pth")
    print("✅ 训练完成，模型已保存为 brain_model.pth")
